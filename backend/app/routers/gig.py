...
def create_gig(
    ...
    new_gig = Gig(**gig_data.model_dump(exclude={"skill_ids"}), client_id=user_id, status=GigStatus.OPEN)
    db.add(new_gig)
    db.commit()
    db.refresh(new_gig)
    ...
    if gig_data.skill_ids:
        for skill_id in gig_data.skill_ids:
            gig_skill = GigSkill(gig_id=new_gig.id, skill_id=skill_id)
            db.add(gig_skill)
        db.commit()
...
def delete_gig(gig_id: int, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    user_id = int(current_user["user_id"])
    gig = db.query(Gig).filter(Gig.id == gig_id).first()
...
def save_gig(gig_id: int, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    user_id = int(current_user["user_id"])
    existing = db.query(SavedGig).filter(SavedGig.user_id == user_id, SavedGig.gig_id == gig_id).first()
    if existing:
...
def get_saved_gigs(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    user_id = int(current_user["user_id"])
    saved = db.query(SavedGig).filter(SavedGig.user_id == user_id).all()
    return [GigResponse.model_validate(s.gig) for s in saved]
...
def get_skills(search: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    return [{"id": s.id, "name": s.name, "category": s.category} for s in query.all()]